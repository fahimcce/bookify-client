...
import { Button, Col, FormProps, InputNumber, Modal, Select } from "antd";
...
const BookingModal = ({ room }: { room: TRoomData }) => {
  ...
  const onFinish: FormProps<TbookingForm>["onFinish"] = async (data) => {
    ...
    if (addbooking?.payload) {
      ...
      toast.success("Please pay now to confirm the booking");
      navigate("/checkout");
    }
  };
  ...
  slots?.map((slots: Tsolts) => {
    ...
    const formattedAvailableDate = dayjs(slots?.date).format("YYYY-MM-DD");
  ...
  const disabledDate: RangePickerProps["disabledDate"] = (current) => {
    const formattedCurrentDate = dayjs(current).format("YYYY-MM-DD");
  ...
  return (
    <>
      <Button
        type="primary"
        onClick={showModal}
        className="bg-blue-600 hover:bg-blue-700 text-white font-semibold w-full px-4 md:px-6 py-2 rounded-lg"
      >
        Book Now
      </Button>
      <Modal
        ...
        open={isModalOpen}
        footer={null} // Removed default Cancel and OK buttons
        onCancel={() => setIsModalOpen(false)} // Close on backdrop click
      ...
      >
        ...
        >
          ...
          {/* Centered "Go for Booking" Button */}
